...
axis = ax[0]
sim, col, num, mes = load2('Results/AWS/low/IZH.csv')
...
axis = ax[1]
sim, col, num, mes = load2('Results/AWS/IZH.csv')
for s in np.flip(np.unique(sim)):
    idx = sim==s
    x = np.sort(np.unique(num[idx]))
    y = np.array([np.mean(mes[idx][num[idx]==n]) for n in x])
...
axis = ax[2]
sim, col, num, mes = load2('Results/AWS/high/IZH.csv')
for s in np.flip(np.unique(sim)):
    idx = sim==s
    x = np.sort(np.unique(num[idx]))
    y = np.array([np.mean(mes[idx][num[idx]==n]) for n in x])
...
# (bbox_to_anchor=(0., 1.02, 1., .102), loc='lower left', ncols=5, mode="expand", borderaxespad=0)
handles, labels = ax[0].get_legend_handles_labels()
new_label = [x[2:] for x in labels]
new_label = [x.removeprefix('⋅').removeprefix(' ') for x in new_label]

axis.text(5900, 0.0013, "Number of Neurons")

# fig.legend(handles, new_label, ncols=5, mode="expand", loc='outside lower center')
fig.legend(handles, new_label, ncols=5, loc='outside lower center')
...
plt.savefig('IZH_rate_legend.jpg', dpi=600)
plt.show()
