...
axis = ax
sim_col, data = load('Results/AWS/Simple2.csv')
...
axis = ax[0]
axis.axvline(10000, c='lightgray', linestyle='--')
...
axis = ax[1]
sim_col, data = load('Results/AWS/Izhikevich.csv')
...
axis = ax[1]
sim_col, data = load('Results/AWS/Simple.csv')

avg_measurements = np.mean(data, axis=0)
brian_mean = avg_measurements[3]
avg_speed_ups = brian_mean/avg_measurements
speed_up_err = np.std(brian_mean/data, axis=0)
